...
public class MySQLSchemaOperatorE2E {
  ...
  public void test() throws IOException {
    ...
    if ("true".equals(System.getenv("RUN_OPERATOR_IN_TEST"))) {
      ...
      operator.register(new MySQLSchemaController(client, dbConfig));
      operator.start();
    }
  ...
  private void deployMySQLServer(KubernetesClient client) throws IOException {
    ...
    Deployment deployment = mapper.readValue(new File("k8s/mysql-deployment.yaml"), Deployment.class);
    ...
    await().atMost(5, MINUTES).until(() -> {
      Deployment mysqlDeployment = client.apps().deployments().inNamespace(TEST_NS).withName(deployment.getMetadata().getName()).get();
      return mysqlDeployment.getStatus().getReadyReplicas() != null && mysqlDeployment.getStatus().getReadyReplicas() == 1;
    });
  }
